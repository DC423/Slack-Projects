#!/usr/bin/python
# Author: Stephen Hilt
# Purpose: Allow DC423 to use nmap commands in slack
# Release: v0.0.0.0.0.0.0.1
import time
import json
import os
import urllib
...
def post_slack(API, chid, message):
    url = "https://slack.com/api/chat.postMessage?token={}&channel={}&text={}&as_user=True&pretty=1".format(
        API, chid, message)
...
def print_scan(nmap_report):
    forreturn = ""
    # Custom Nmap starting message, this wad done to add DC423, this can be
    # changed to your own slack
    forreturn = "Starting Nmap DC423 ( http://nmap.org ) at {}".format(
        nmap_report.started)
...
while True:
  ...
  # if the command entered contains run nmap, and does not contain USAGE
    ...
    # Retreive the user name so we can log who it was that
    # successfully submitted a scan
    url = "https://slack.com/api/users.info?token={}&user={}&pretty=1".format(
       API, j_result['user'])
    u = urllib.urlopen(url)
    response = u.read()
    response = json.loads(response)
    ...
    # if there is only two (aka run nmap) was all that was entered,
    # then print usage
    ...
    # else do all this other stuff
			...
			cmd = j_result['text']
      # if there is http in the commad, likely a domain name was
      # passed in to slack
      if "http" in cmd:
